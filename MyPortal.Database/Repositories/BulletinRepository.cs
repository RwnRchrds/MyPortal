...
{SqlHelper.Join(JoinType.LeftJoin, "[dbo].[AspNetUsers]", "[User].[Id]", "[Bulletin].[AuthorId]", "User")}";
        ...
        public async Task<IEnumerable<Bulletin>> GetAll()
        {
        ...
        public async Task<Bulletin> GetById(Guid id)
        {
            var sql = $"SELECT {AllColumns},{RelatedColumns} FROM {TblName} {JoinRelated}";
            
            SqlHelper.Where(ref sql, "[Bulletin].[Id] = @BulletinId");

            return (await ExecuteQuery(sql, new {BulletinId = id})).Single();
        }
