...
namespace MyPortal.Services
{
    public static class SystemService
    {
        ...
        public static async Task<ProcessResponse<object>> CreateBulletin(SystemBulletin bulletin, string userId, MyPortalDbContext context, bool autoApprove = false)
        {
            ...
            var author = await StaffMemberService.GetAuthor(userId, authorId, context);
        ...
        public static async Task<int> GetCurrentOrSelectedAcademicYearId(MyPortalDbContext context, IPrincipal user)
        {
            ...
            if (user != null && await user.HasPermissionAsync("ChangeAcademicYear"))
            {
        ...
        {
            var author = context.StaffMembers.SingleOrDefault(x => x.Person.UserId == userId);

            var bulletins = GetOwnBulletins_Model(author.Id, context).ResponseObject
                .Select(Mapper.Map<SystemBulletin, SystemBulletinDto>);
            return new ProcessResponse<IEnumerable<SystemBulletinDto>>(ResponseType.Ok, null, bulletins);
        }
